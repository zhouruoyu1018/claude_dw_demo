...
仅当拆分后的最小语义单元在预取结果中仍**找不到**时，才单独调用 `search_word_root(keyword="xxx")` 精确搜索。如果仍无结果，使用通用英文缩写并在 COMMENT 中标注"（新词根，待入库）"。
...
所有指标字段命名完成后，**必须逐字段执行以下自检**，确认词根 tag 顺序正确。
...
1. **拆解词根**：将字段名拆分为各组成词根
2. **标注 tag**：为每个词根标注其在词根表中的 tag 类型
3. **校验顺序**：确认 tag 出现顺序符合 `BOOL → TIME → CONVERGE → BIZ_ENTITY → CATEGORY_WORD`
4. **标注结果**：✅ 通过 / ❌ 顺序违规
...
│ 字段名             │ 词根拆解 (tag)                   │ 结果  │
├────────────────────────────────────────────────────────────────┤
│ td_sum_loan_amt    │ td(TIME)+sum(CONV)+loan(BIZ)+    │ ✅    │
│                    │ amt(CAT)                         │       │
│ m0_enr_mtd         │ m0(CAT)+enr(CAT)+mtd(CAT)       │ ✅    │
│                    │ (同类CAT: 核心度量在前,范围在后)   │       │
│ mtd_m0_enr         │ mtd(CAT)+m0(CAT)+enr(CAT)       │ ❌    │
│                    │ → 应改为 m0_enr_mtd               │       │
└────────────────────────────────────────────────────────────────┘
```
...
| TIME 不在开头 | `loan_td_amt` | `td_loan_amt` | td 是 TIME，应在 CONVERGE 之前 |
| CONVERGE 在 BIZ_ENTITY 之后 | `loan_sum_amt` | `sum_loan_amt` | sum 是 CONVERGE，应在 BIZ_ENTITY 之前 |
...
| 3 | 并行预取5类词根 → 本地匹配 | `td_sum_loan_amt`, `td_cnt_loan`, `td_avg_credit_amt` |
