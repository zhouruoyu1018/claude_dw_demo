...
COMMENT ON COLUMN data_lineage.target_table IS '目标表完整名，如 ph_sac_dmm.dmm_sac_loan_prod_daily';
...
-- 场景1: ph_sac_dmm.dmm_sac_loan_prod_daily (贷款产品日报表)
...
VALUES
('ph_sac_dmm.dmm_sac_loan_prod_daily', 'ph_sac_dmm', 'dwd.dwd_loan_detail', 'dwd', 'ETL', 'FROM', 'sql/hive/etl/dm/dmm_sac_loan_prod_daily_etl.sql', '按产品维度聚合当日放款明细', 'test_user'),
('ph_sac_dmm.dmm_sac_loan_prod_daily', 'ph_sac_dmm', 'dim.dim_product', 'dim', 'ETL', 'LEFT JOIN', 'sql/hive/etl/dm/dmm_sac_loan_prod_daily_etl.sql', '关联产品维度获取产品名称', 'test_user');

-- 场景2: ph_sac_dmm.dmm_sac_loan_chn_daily (贷款渠道日报表)
--   依赖: dwd.dwd_loan_detail, dim.dim_channel
INSERT INTO data_lineage (target_table, target_schema, source_table, source_schema, relation_type, join_type, etl_script_path, etl_logic_summary, created_by)
VALUES
('ph_sac_dmm.dmm_sac_loan_chn_daily', 'ph_sac_dmm', 'dwd.dwd_loan_detail', 'dwd', 'ETL', 'FROM', 'sql/hive/etl/dm/dmm_sac_loan_chn_daily_etl.sql', '按渠道维度聚合当日放款明细', 'test_user'),
('ph_sac_dmm.dmm_sac_loan_chn_daily', 'ph_sac_dmm', 'dim.dim_channel', 'dim', 'ETL', 'LEFT JOIN', 'sql/hive/etl/dm/dmm_sac_loan_chn_daily_etl.sql', '关联渠道维度获取渠道名称', 'test_user');

-- 场景3: ph_sac_da.da_loan_report (贷款报表 - 依赖 dm 层)
--   依赖: ph_sac_dmm.dmm_sac_loan_prod_daily, ph_sac_dmm.dmm_sac_loan_chn_daily
INSERT INTO data_lineage (target_table, target_schema, source_table, source_schema, relation_type, join_type, etl_script_path, etl_logic_summary, created_by)
VALUES
('ph_sac_da.da_loan_report', 'ph_sac_da', 'ph_sac_dmm.dmm_sac_loan_prod_daily', 'ph_sac_dmm', 'ETL', 'FROM', 'sql/hive/etl/da/da_loan_report_etl.sql', '汇总产品维度指标到报表层', 'test_user'),
('ph_sac_da.da_loan_report', 'ph_sac_da', 'ph_sac_dmm.dmm_sac_loan_chn_daily', 'ph_sac_dmm', 'ETL', 'LEFT JOIN', 'sql/hive/etl/da/da_loan_report_etl.sql', '关联渠道维度指标', 'test_user');
...
-- ph_sac_dmm.dmm_sac_loan_prod_daily 的字段血缘
...
('ph_sac_dmm.dmm_sac_loan_prod_daily', 'today_loan_cnt', 'dwd.dwd_loan_detail', 'loan_id', 'COUNT', 'COUNT(loan_id)', 1),
...
('ph_sac_dmm.dmm_sac_loan_chn_daily', 'today_loan_cnt', 'dwd.dwd_loan_detail', 'loan_id', 'COUNT', 'COUNT(loan_id)', 3);
...
('ph_sac_da.da_loan_report', 'total_loan_cnt', 'ph_sac_dmm.dmm_sac_loan_prod_daily', 'today_loan_cnt', 'SUM', 'SUM(today_loan_cnt)', 5);
...
-- 测试上游查询
SELECT '=== 测试: ph_sac_dmm.dmm_sac_loan_prod_daily 的上游依赖 ===' AS info;
