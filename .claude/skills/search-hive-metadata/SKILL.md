...
- **查询类工具**（search_table、search_by_comment、get_table_detail、list_columns、search_word_root、search_word_root_batch、assemble_field_names、validate_field_name、validate_field_names、search_existing_indicators、search_lineage_*）：告知用户 MCP 不可用，请求用户手动提供所需信息，然后继续流程
...
| `search_word_root_batch` | 查询 | **优先使用** — 批量查询多个语义单元词根，最多 2 次 DB 查询（≥3 个语义单元时优先使用） |
| `validate_field_name` | 查询 | 校验单个字段名词根存在性、tag 顺序和命名证据覆盖 |
| `assemble_field_names` | 查询 | **优先使用** — 按 tag 顺序规则化拼接字段名，输入 roots+tags 输出确定性字段名，可选 DB 校验 |
| `validate_field_names` | 查询 | **优先使用** — 批量校验多个字段名（单次 DB 查询），含跨字段重名检测 + 字段数完整性检查，DDL 输出前必调 |
...
1. **语义拆分**：将所有字段的业务含义拆为最小语义单元，跨字段去重
2. **批量词根查询**：将去重后的语义单元调用 `search_word_root_batch` 一次性查询（≥3 个语义单元时），结果跨字段复用；降级时并行调用 `search_word_root`
3. **field_spec 结构化命名**：模型输出 `field_spec`（语义单元 → 词根 → tag），再按 tag 顺序规则化拼接字段名
4. **批量校验**：调用 `validate_field_names` 一次性校验全部字段（含重名检测）；批量工具不可用时降级为并行调用 `validate_field_name`
5. 若校验失败，仅修正失败字段并重试，禁止整表重跑；全部通过后才输出 DDL

> ⚠️ 禁止按 tag 全量浏览词根表（1000+ 条，limit=50 无法覆盖且浪费上下文）。仅查询本次字段涉及的语义单元。
