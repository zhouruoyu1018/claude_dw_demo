...
| today_loan_amt     | dwd_loan_detail.loan_amount    | SUM(loan_amount)             | SUM     |
| today_loan_cnt     | dwd_loan_detail.loan_id        | COUNT(DISTINCT loan_id)      | COUNT   |
| avg_loan_amt    | -                              | today_loan_amt / today_loan_cnt    | CUSTOM  |

### 3️⃣ 识别的指标

| 指标英文名        | 指标中文名      | 计算逻辑                       | 标准类型  | 分类     |
|------------------|----------------|-------------------------------|---------|---------|
| today_loan_amt      | 当日放款金额    | SUM(loan_amount)              | 数值类   | 原子指标 |
| today_loan_cnt      | 当日放款笔数    | COUNT(DISTINCT loan_id)       | 数值类   | 原子指标 |
| avg_loan_amt     | 平均单笔金额    | today_loan_amt / today_loan_cnt     | 数值类   | 派生指标 |
